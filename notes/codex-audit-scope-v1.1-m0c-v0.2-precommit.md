...
### §1.5 历史文档豁免清单（继承 v0.1 §1.5 13 文件 42 处 + v0.2 audit-scope 自伤 ≥ 1）

继承 v0.1 §1.5 13 文件 42 处三类定性（① 守门字面自伤 / ② 叙述性引用 / ③ 署名尾注），新增：
...
**豁免口径不变**（per M0b plan §3 L89 C2 裁定 + pre-commit report §2 M1）：**不清洗历史文档**（考古记录 + git 尾注保留）。
